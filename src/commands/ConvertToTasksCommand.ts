...
export class ConvertToTasksCommand {
    static async execute(uri?: vscode.Uri, outputChannel?: vscode.OutputChannel) {
        ...
        try {
            const prompt = await PromptGenerator.generateConvertToTasksPrompt(decisionUri);

            if (outputChannel) {
                outputChannel.clear();
                outputChannel.appendLine('='.repeat(80));
                outputChannel.appendLine('GLAM: Convert Decision to Tasks');
                outputChannel.appendLine('='.repeat(80));
            ...
            vscode.window.showInformationMessage(
                'Convert to tasks prompt generated! Check the Glam output panel.'
            );
        } catch (error) {
    ...
    private static async selectDecisionFile(): Promise<vscode.Uri | undefined> {
        ...
        try {
            ...
            const selected = await vscode.window.showQuickPick(decisionFiles, {
                placeHolder: 'Select a decision file to convert to tasks'
            });
